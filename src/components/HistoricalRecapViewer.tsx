...
export const HistoricalRecapViewer: React.FC<HistoricalRecapViewerProps> = ({ onReportSelect }) => {
  ...
  useEffect(() => {
    ...
    if (data.length > 0) {
      ...
      setFilter(prev => ({
        ...
        dateTo: latest.toISOString().split('T')[0]
      }));
    }
  }, []);
        ...
        <Box sx={{ height: 600, width: '100%' }}>
          <div className="ag-theme-alpine" style={{ height: '100%', width: '100%' }}>
            <AgGridReact
              ...
              rowSelection="single"
              onRowClicked={(event) => {
                const report = historicalData.find(r => r.date === event.data.date);
